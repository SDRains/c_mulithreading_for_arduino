...
void CountUp(void);					// Starting at 1, count up
void port_main(void);				// Starts the port communication functions
void initScreen(void);				// Initialize the screen for program
void CloseThread(void);				// Close current thread
...
int main() // Thread One
{
	// Initialize the program and screen
	initScreen();
...
void initScreen() {
	...
	printf("Welcome to multithreading in C\n");
	printf("Press 'a' to watch letters bounce.\n");
	printf("Press 'c' to count.\n");
	printf("Press 'p' to open Comm port\n");
}
...
void KbdFunc(void) // Dispatch and count threads.
{
	...
	do
	{
		...
		if (tolower(KeyInfo) == 'c' && ThreadNr < MAX_THREADS) {
			ThreadNr++;
			_beginthread(CountUp, 0, &ThreadNr);
			WriteTitle(ThreadNr);
		}
		if (tolower(KeyInfo) == 'p' && ThreadNr < MAX_THREADS) {
			ThreadNr++;
			// _beginthread(port_main, 0, &ThreadNr);
			WriteTitle(ThreadNr); 
			// While the ability to open more threads exists, it is shut down and inaccessable inside the next function
			port_main();
			initScreen();
		}
